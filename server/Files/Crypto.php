<?php

namespace ANCENC\Files;

class Crypto {

	private $public_key;
	private $private_key;
	private $public_path = ANCENC_PATH . DIRECTORY_SEPARATOR . 'keys' . DIRECTORY_SEPARATOR . 'public.pem';
	private $private_path = ANCENC_PATH . DIRECTORY_SEPARATOR . 'keys' . DIRECTORY_SEPARATOR . 'private.pem';

	public function create_keys() {
		$rsa = new \Pikirasa\RSA( null, null );
		$rsa->create();
		$this->public_key  = $rsa->getPublicKeyFile();
		$this->private_key = $rsa->getPrivateKeyFile();

		$this->public_key = '
-----BEGIN PUBLIC KEY-----

FwIDAQAB
-----END PUBLIC KEY-----
';

		$this->private_key = '
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
';
	}

	public function encrypt( $data ) {
		$rsa = new \Pikirasa\RSA( $this->public_path, $this->private_path );
		return $rsa->encrypt( base64_decode($data) );
	}

	public function decrypt( $data ) {
		$rsa = new \Pikirasa\RSA( $this->public_path, $this->private_path );
		return $rsa->decrypt( $data );
	}

	public function write_keys() {

		if ( ! file_exists( $this->public_path ) ) {
			touch( $this->public_path );
		}

		if ( ! file_exists( $this->private_path ) ) {
			touch( $this->private_path );
		}

		$this->create_keys();

		$public_file  = fopen( $this->public_path, 'w+' );
		$private_file = fopen( $this->private_path, 'w+' );

		fwrite( $public_file, $this->public_key );
		fwrite( $private_file, $this->private_key );

		fclose( $public_file );
		fclose( $private_file );
	}
}